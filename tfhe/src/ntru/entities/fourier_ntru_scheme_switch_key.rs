...
pub struct FourierNtruSchemeSwitchKey<C: Container<Element = c64>> {
    fourier: FourierPolynomialList<C>,
    decomp_base_log: DecompositionBaseLog,
    fft_type: FftType,
}

pub type FourierNtruSchemeSwitchKeyView<'a> = FourierNtruSchemeSwitchKey<&'a [c64]>;
pub type FourierNtruSchemeSwitchKeyMutView<'a> = FourierNtruSchemeSwitchKey<&'a mut [c64]>;

impl<C: Container<Element = c64>> FourierNtruSchemeSwitchKey<C> {
    ...
    pub fn as_view(&self) -> FourierNtruSchemeSwitchKeyView<'_>
    where
        C: AsRef<[c64]>
    {
        FourierNtruSchemeSwitchKeyView {
            fourier: FourierPolynomialList {
    ...
    pub fn as_mut_view(&mut self) -> FourierNtruSchemeSwitchKeyMutView<'_>
    where
        C: AsMut<[c64]>,
    {
        FourierNtruSchemeSwitchKeyMutView {
            fourier: FourierPolynomialList {
    ...
    pub fn as_mut_fourier_ntru_keyswitch_key(&mut self) -> FourierNtruKeyswitchKeyMutView<'_>
    where
        C: AsMut<[c64]>,
    {
        FourierNtruKeyswitchKey::from_container(
...
type FourierNtruSchemeSwitchKeyOwned = FourierNtruSchemeSwitchKey<ABox<[c64]>>;

impl FourierNtruSchemeSwitchKeyOwned {
    pub fn new(
        polynomial_size: PolynomialSize,
        decomp_base_log: DecompositionBaseLog,
        ...
        FourierNtruSchemeSwitchKey::from_container(
            boxed,
            polynomial_size,
            decomp_base_log,
            fft_type,
