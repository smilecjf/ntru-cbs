//! Module containing the definition of the NtruSchemeSwitcKey
...
pub struct NtruSchemeSwitchKey<C: Container>
where
...
impl<T: UnsignedInteger, C: Container<Element = T>> AsRef<[T]> for NtruSchemeSwitchKey<C> {
    fn as_ref(&self) -> &[T] {
        self.data.as_ref()
    }
}

impl<T: UnsignedInteger, C: ContainerMut<Element = T>> AsMut<[T]> for NtruSchemeSwitchKey<C> {
    fn as_mut(&mut self) -> &mut [T] {
        self.data.as_mut()
    }
}

pub type NtruSchemeSwitchKeyOwned<Scalar> = NtruSchemeSwitchKey<Vec<Scalar>>;
pub type NtruSchemeSwitchKeyView<'data, Scalar> = NtruSchemeSwitchKey<&'data [Scalar]>;
pub type NtruSchemeSwitchKeyMutView<'data, Scalar> = NtruSchemeSwitchKey<&'data mut [Scalar]>;

impl<Scalar: UnsignedInteger, C: Container<Element = Scalar>> NtruSchemeSwitchKey<C> {
    ...
    pub fn as_view(&self) -> NtruSchemeSwitchKeyView<'_, Scalar> {
        NtruSchemeSwitchKey::from_container(
            self.data.as_ref(),
            self.polynomial_size,
            self.decomp_base_log,
            self.ciphertext_modulus,
...
impl<Scalar: UnsignedInteger, C: ContainerMut<Element = Scalar>> NtruSchemeSwitchKey<C> {
    pub fn as_mut_view(&mut self) -> NtruSchemeSwitchKeyMutView<'_, Scalar> {
        NtruSchemeSwitchKey::from_container(
            self.data.as_mut(),
            self.polynomial_size,
            self.decomp_base_log,
...
impl<Scalar: UnsignedInteger> NtruSchemeSwitchKeyOwned<Scalar> {
    pub fn new(
        fill_with: Scalar,
        polynomial_size: PolynomialSize,
        decomp_base_log: DecompositionBaseLog,
