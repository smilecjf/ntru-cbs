...
pub fn generate_ntru_scheme_switch_key<
...
>(
    ...
    ntru_scheme_switch_key: &mut NtruSchemeSwitchKey<KeyCont>,
...
)
{
    ...
    assert_eq!(
        ...
        ntru_scheme_switch_key.ciphertext_modulus(),
    );
    ...
    generate_ntru_keyswitch_key(
        &input_ntru_secret_key,
        &ntru_secret_key_one,
        &mut ntru_scheme_switch_key.as_mut_ntru_keyswitch_key(),
        noise_distribution,
        generator,
    );
}

pub fn allocate_and_generate_new_ntru_scheme_switch_key<
    Scalar: Encryptable<Uniform, NoiseDistribution>,
    NoiseDistribution: Distribution,
    InputKeyCont: Container<Element = Scalar>,
    Gen: ByteRandomGenerator,
>(
    ...
    noise_distribution: NoiseDistribution,
    generator: &mut EncryptionRandomGenerator<Gen>,
) -> NtruSchemeSwitchKeyOwned<Scalar>
{
    let mut new_ntru_scheme_switch_key = NtruSchemeSwitchKey::new(
        Scalar::ZERO,
        input_ntru_secret_key.polynomial_size(),
        decomp_base_log,
        decomp_level_count,
    ...
    generate_ntru_scheme_switch_key(
        input_ntru_secret_key,
        &mut new_ntru_scheme_switch_key,
        noise_distribution,
        generator,
    );

    new_ntru_scheme_switch_key
}
