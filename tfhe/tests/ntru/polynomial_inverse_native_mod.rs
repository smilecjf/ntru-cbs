...
pub fn main() {
    ...
    for idx in 0..num_test {
        ...
        for coeff in x.as_mut().iter_mut() {
            *coeff = rand::thread_rng().gen_range(0..=Scalar::MAX);
        }

        if polynomial_size.0 <= 16 {
            println!("x: {:}", polynomial_to_string_native_mod(&x));
        }

        let invertible = polynomial_inverse_mod_power_of_two(&x, &mut y, Scalar::BITS as usize);

        if invertible {
            if polynomial_size.0 <= 16 {
                println!("y: {:}", polynomial_to_string_native_mod(&y));
            }
