...
pub fn main() {
    ...
    let mut ntru_auto_ciphertext = NtruCiphertext::new(
        Scalar::ZERO,
        polynomial_size,
        ciphertext_modulus,
    );
    ...
    for idx in 1..=num_test {
        ...
        for i in 0..polynomial_size.0 {
            ...
            if i == 0 {
                correct_message_list.as_mut()[0] = rand_msg;
            } else {
                let j = i * index % polynomial_size.0;
                if (i * index) / polynomial_size.0 % 2 == 0 {
                    correct_message_list.as_mut()[j] = rand_msg;
                } else {
                    correct_message_list.as_mut()[j] = message_modulus.wrapping_sub(rand_msg);
                }
            }
        }
